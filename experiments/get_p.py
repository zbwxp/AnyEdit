...
from util.multi_layer_stats import multi_layer_stats
...
def main(
    ...
    if any(alg in alg_name for alg in ["AlphaEdit","AlphaEdit_ARE"]):
        if not os.path.exists(f"{hparams.model_name}_null_space_project.pt"):
            # Compute null-space projectors for all layers in a single dataset pass
            P = get_projects(model, tok, hparams)
...
def get_projects(model, tok, hparams):
    """
    Compute null-space projectors for all layers in hparams.layers in a single pass
    over the dataset by collecting multi-layer covariance statistics.
    Returns a tensor P of shape (num_layers, d, d) on CPU.
    """
    layer_names = [hparams.rewrite_module_tmp.format(l) for l in hparams.layers]
    stats = multi_layer_stats(
        model,
        tok,
        layer_names,
        STATS_DIR,
    ...
    d = W_out.shape[1]
    P = torch.zeros((len(hparams.layers), d, d), device="cpu")
    for i, layer in enumerate(hparams.layers):
        ln = hparams.rewrite_module_tmp.format(layer)
        cov = stats[ln].mom2.moment().float().to("cuda")
        U, S, _ = torch.linalg.svd(cov, full_matrices=False)
        threshold = hparams.nullspace_threshold
        small_singular_indices = (S < threshold).nonzero(as_tuple=True)[0]
        P_i = (U[:, small_singular_indices] @ U[:, small_singular_indices].T).to("cpu")
        P[i, :, :] = P_i
    return P
